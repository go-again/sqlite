...
- `vfs/compress/` — a SQLite database compressed at rest: `compress.Open` inflates the file into a temp working copy, opens it as a normal DB, and recompresses on `db.Close()` via the `Config.VFSCloser` seam (a **snapshot** model — per-session durability, not a live VFS; the working copy is plaintext); also `Pack`/`Unpack` file transforms. A **separate module** (`gosqlite.org/vfs/compress`, its own `go.mod` + `replace gosqlite.org => ../..`) so its codec dep stays out of the root graph; example under `vfs/compress/example/`. Coverage: [`dev/coverage/compress.md`](dev/coverage/compress.md). Live transparent compression (and combining it with crypto) is a separate, larger effort — a page-translation VFS.
