...
- `vfs/` — `vfs.New(fs.FS)` + `vfs.NewReader(io.ReaderAt, size)`; the public user-implementable VFS interface (`vfs.Register(name, VFS)` with `vfs.VFS` / `vfs.File`, optional `vfs.ShmFile` for WAL, `vfs.NoLock`, `vfs.Wrap(base, Recorder)` instrumentation; dispatcher in `register.go` / `iomethods.go` / `shm.go`); `vfs/crypto/` (encryption-at-rest), `vfs/cksm/` (page-checksum trailer), `vfs/mvcc/` (in-memory snapshot isolation), `vfs/memdb/` (plain in-memory).
- `pcache/` — application-controlled page cache. `pcache.InstallBoundedLRU(maxPages)` swaps SQLite's page cache (`SQLITE_CONFIG_PCACHE2`) for a bounded LRU over C-allocated page blocks with hit/miss/eviction/live counters (`Stats`). One-time, process-global, install before the first `sql.Open`. Off-heap page memory + the 11 PCACHE2 trampolines via `internal/cabi`.
...
- `examples/` — runnable examples, grouped by reader intent: `migrating/` (drop-in from mattn / modernc / glebarez), `getting-started/` (the modern `database/sql` + typed `Config` + gorm on-ramp), and `features/{search,vfs,extensions,advanced,gorm}/` (capability reference). `examples/README.md` is the two-audience router; each group has its own index README. Smoke-tested by `just examples` (depth-agnostic `find main.go`); run one with `just example <leaf-or-subpath>`. Add a new example under the group that fits; raw SQL is fine in a feature example (the feature is conn-level), the legacy `sqlite3`-driver / `_`-DSN style belongs only under `migrating/`.
