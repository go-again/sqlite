...
type Config struct {
	// Path is the on-disk file path. Use ":memory:" for an in-
	// memory database. Required.
	Path string
...
type Pragmas struct {
	...
	// or "OFF". Empty = leave alone (SQLite default DELETE).
	JournalMode string
	...
	// Synchronous: "OFF", "NORMAL", "FULL", "EXTRA". Empty = leave alone.
	Synchronous string
	...
	// TempStore: "DEFAULT", "FILE", "MEMORY". Empty = leave alone.
	TempStore string
