...
func newConn(dsn string) (*conn, error) {
	...
	if pos >= 1 {
		query = dsn[pos+1:]
		var err error
		query, err = translateMattnDSN(query)
		if err != nil {
			return nil, err
		}
		...
		if !strings.HasPrefix(dsn, "file:") {
			...
			// Rewrite the file: URI's query string to the normalized form so
			// the URI parser inside SQLite sees the mattn-translated flags too
			// (necessary for things like cache=shared and mode=memory).
			dsn = dsn[:pos+1] + query
		}
	}
