...
| `GenerateX25519()` | ✓ typed | `TestMultiRecipientSSH` (helpers), used by `vfs/compress` recipient tests | A fresh native age keypair — key-based access without SSH or a shared passphrase. |
| `ErrNoMatch` | ✓ typed | `TestMultiRecipientSSH` | Returned by `Unwrap` when no supplied identity matches; mapped to `compress.ErrNoIdentity` at the VFS boundary. Test with `errors.Is`. |
...
| Masters: `ResealKeyslot` gate | ✓ typed | (via `compress.TestMasterModel`, `TestRemoveMaster`) | Rewriting a master-protected membership requires a current master (`ErrNotMaster`) and cannot drop all masters; flat→master upgrade requires the actor to be one of the new masters. |
