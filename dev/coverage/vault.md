...
- **Online (in-place) shrink** — under churn the container reuses freed blocks (the at-rest file plateaus rather than growing); it does not shrink the physical file mid-session. Reclaiming space to the OS is the offline `Compact` (a container→container rewrite on a closed database that continues the generation, so an `Options.Anchor` stays valid) — not an online/mid-session operation.
