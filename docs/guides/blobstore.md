...
The storage *mode* (raw vs compressed) is fixed at `Create`, but the *level* is changeable with `store.SetCompression(ctx, id, level)` — write a head at `CompressionBest`, then lower the level before appending a large tail; chunks at different levels read back fine. `store.Stat(ctx, id)` returns an object's metadata — logical size, stored bytes, the actual at-rest compression **ratio** (computed from the chunk sizes, not a maintained column), chunk size, mode, and current level.
