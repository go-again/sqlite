...
//
// The Code accessor returns the primary SQLite result code (low byte). For the
// full extended code (e.g. SQLITE_CONSTRAINT_UNIQUE = SQLITE_CONSTRAINT |
// (8 << 8)), use ExtendedCode.
//
// Mattn-compatibility note: mattn's Error struct exposes Code/ExtendedCode as
// exported fields of type ErrNo / ErrNoExtended; this struct surfaces them as
// methods to keep field-level compatibility cross-driver. Code that does
// `if e.Code() == sqlite.SQLITE_CONSTRAINT` works the same as mattn's
// `if e.Code == sqlite3.ErrConstraint` (compare against ErrConstraint type-cast).
type Error struct {
	msg  string
	code int // extended SQLite result code
}

// Error implements error.
func (e *Error) Error() string { return e.msg }

// Code returns the primary SQLite result code for this error (low byte of the
// extended code). Compare against the SQLITE_* constants.
func (e *Error) Code() int { return e.code & 0xff }

// ExtendedCode returns the extended SQLite result code for this error (e.g.
// SQLITE_CONSTRAINT_UNIQUE). Compare against ErrConstraint* constants or the
// SQLITE_CONSTRAINT_* family.
func (e *Error) ExtendedCode() int { return e.code }

// Is implements errors.Is. An Error matches another Error if their primary
// codes match. It also matches a bare ErrNo or ErrNoExtended.
