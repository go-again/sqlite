...
| [`gorm`](gorm/) | gorm opened from the same `sqlite.Config` via `sqlitegorm.OpenConfig` — the recommended gorm entry. |

From here, [`../features/`](../features/) has a reference example for every capability (vector / FTS5 search, custom VFS, the `ext/` catalog, hooks, sessions, …). For the higher-level typed surfaces specifically, see [`../features/search/`](../features/search/) (vec / FTS5) and [`../features/gorm/`](../features/gorm/) (tag-driven sidecars).
