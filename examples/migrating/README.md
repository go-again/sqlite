...
| [`from-glebarez`](../../gorm/examples/from-glebarez/) | `glebarez/sqlite` or `gorm.io/driver/sqlite` | swap the gorm dialector import to `gosqlite.org/gorm` (its own module); the `sqlite.Open(dsn)` call signature is identical, and `TranslateError` behaves the same. |
