...
func openDB(t *testing.T) (*sql.DB, *sql.Conn) {
	...
	// unicode.Register is variadic, so it does not satisfy the
	// func(*sqlite.Conn) error shape RegisterOn expects directly.
	testhelp.RegisterOn(t, sc, func(c *sqlite.Conn) error { return unicode.Register(c) })
	return db, sc
}
