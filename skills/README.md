# Skills — for AI agents using go-again/sqlite

These are [Agent Skills](https://docs.claude.com/en/docs/agents-and-tools/agent-skills/overview): task-scoped instructions that teach an AI agent how to **use** `github.com/go-again/sqlite` correctly when building an application with it. Each skill is a folder with a `SKILL.md` whose frontmatter `description` says when to load it; the agent reads the description, then pulls in the body when the task matches.

| Skill | Load it when the task is… |
|---|---|
| [`using-go-again-sqlite`](using-go-again-sqlite/SKILL.md) | opening a database / picking a driver name / choosing a sub-package (start here) |
