...
func Rewrap(path string, by keyring.Identity, writeAs keyring.WriterIdentity, m keyring.Membership) error {
	...
	if c.keyslotOffset == 0 {
		return errors.New("compress: not a recipients-encrypted database (nothing to rewrap)")
	}
...
func (c *container) applyMembership(m keyring.Membership, writeAs keyring.WriterIdentity) error {
	...
	case c.authenticated:
		if len(m.Writers) == 0 {
			return errors.New("compress: an authenticated database must keep at least one writer")
		}
		if !keyring.WriterAuthorized(m.Writers, writeAs) {
			return errors.New("compress: WriteAs must be one of the new Writers to re-sign the database")
		}
		c.writeAs = writeAs
	case len(m.Writers) > 0:
		return errors.New("compress: cannot add writers to a non-authenticated database (recreate it with Options.Writers)")
	}
	return nil
}
...
func Rekey(path string, by keyring.Identity, writeAs keyring.WriterIdentity, m keyring.Membership) error {
	...
	defer closeContainer(c, &err)
	if c.keyslotOffset == 0 {
		return errors.New("compress: not a recipients-encrypted database")
	}
	if err = c.applyMembership(m, writeAs); err != nil {
		return err
	}
	...
	if !ok {
		return fmt.Errorf("compress: unknown on-disk cipher marker %d", c.enc)
	}
...
func openAtRest(path string, identity keyring.Identity) (*container, error) {
	...
	if open {
		return nil, fmt.Errorf("compress: %q is open; close it before key management", path)
	}
	...
	if fi.Size() == 0 {
		_ = file.Close()
		return nil, fmt.Errorf("compress: %q is empty, not an encrypted database", path)
	}
