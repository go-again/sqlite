...
type Options struct {
	...
	// slice as soon as New returns. For multi-recipient encryption (a
	// wrapped key, no shared secret) and authenticated/tamper-evident
	// storage, use gosqlite.org/vfs/vault instead.
	Key []byte
...
type FS struct {
	...
	tls   *libc.TLS
	// cipher is built once in New from the raw key and never mutated, so the io
	// trampolines read it without synchronization — it is published before the VFS
	// is registered, which happens-before any open and therefore any I/O.
	cipher   PageCipher
	pageSize int32
