...
// TestValidateDirectoryRejectsOverlap confirms the structural disjointness guard:
// the superblocks, directory, keyslot, and every live slot must occupy distinct
// blocks. A disjoint layout passes; any overlap (slot↔slot, slot↔superblock,
// slot↔directory, slot↔keyslot) is rejected, so a corrupt or re-sealed directory
// cannot drive rebuildAllocator into aliasing a live block.
func TestValidateDirectoryRejectsOverlap(t *testing.T) {
	const fileSize = 1 << 20
	bs := uint64(defaultBlockSize)

	// superblocks [0,2) · directory block 2 · keyslot block 3 · slots at 4 and 5–6.
	okSB := &superblock{
		blockSize: defaultBlockSize, pageSize: defaultPageSize,
		dirOffset: 2 * bs, dirBlocks: 1, keyslotOffset: 3 * bs,
	}
	good := []dirEntry{
		{}, // sparse
