...
func OpenSnapshot(cfg sqlite.Config, opts Options) (*sqlite.DB, error) {
	...
	if fi, err := os.Stat(destDir); err != nil {
		return nil, fmt.Errorf("vault: destination directory %q is not accessible: %w", destDir, err)
	} else if !fi.IsDir() {
		return nil, fmt.Errorf("vault: destination parent %q is not a directory", destDir)
	}

	dest := cfg.Path
	workDir, err := os.MkdirTemp(opts.TempDir, "gosqlite-compress-")
	if err != nil {
		return nil, fmt.Errorf("vault: create working dir: %w", err)
	}
...
func inflate(dest, workPath string, max int64) error {
	...
	if err != nil {
		return fmt.Errorf("vault: open %q: %w", dest, err)
	}
	defer f.Close()
	...
	if err != nil && !errors.Is(err, io.ErrUnexpectedEOF) {
		return fmt.Errorf("vault: read %q: %w", dest, err)
	}
	if _, err := f.Seek(0, io.SeekStart); err != nil {
		return fmt.Errorf("vault: seek %q: %w", dest, err)
	}

	out, err := os.Create(workPath)
	if err != nil {
		return fmt.Errorf("vault: create working copy: %w", err)
	}
	defer out.Close()

	if looksLikeSQLite(hdr[:n]) {
		if _, err := io.Copy(out, f); err != nil {
			return fmt.Errorf("vault: copy %q: %w", dest, err)
		}
	} else {
		dn, err := decompressStream(out, f, max)
		if err != nil {
			return fmt.Errorf("vault: %q is neither a compressed nor a SQLite database: %w", dest, err)
		}
		if dn == 0 {
			...
			return fmt.Errorf("vault: %q is too short to be a compressed or SQLite database", dest)
		}
	}
	return out.Sync()
}
...
func (rc *recompressor) Close() error {
	...
	if err := consolidate(rc.workPath); err != nil {
		return fmt.Errorf("vault: consolidate working copy: %w", err)
	}
	if err := packFile(rc.dest, rc.workPath, rc.level); err != nil {
		return fmt.Errorf("vault: recompress to %q: %w", rc.dest, err)
	}
	return nil
}
